...
console.log('ENV module loading...');
import { CopyStrategy, CopyStrategyConfig, parseTieredMultipliers } from '../strategy/CopyStrategy';
dotenv.config();
...
const validateUrls = (): void => {
    ...
    if (process.env.MONGO_URI && !process.env.MONGO_URI.startsWith('mongodb')) {
        ...
        console.error('Example: mongodb+srv://<user>:<password>@cluster.mongodb.net/database\n');
        throw new Error(
...
validateUrls();
    ...
    const parseBlacklistAddresses = (input: string): string[] => {
        if (!input) return [];
        return input
            .split(',')
            .map((addr) => addr.toLowerCase().trim())
...
const blacklist = parseBlacklistAddresses(process.env.BLACKLIST_ADDRESSES || '');
const userAddresses = parseUserAddresses(process.env.USER_ADDRESSES as string)
    .filter(addr => !blacklist.includes(addr));

if (blacklist.length > 0) {
    console.log(`\n🚫 Blacklisted ${blacklist.length} addresses from configuration.`);
}

export const ENV = {
    USER_ADDRESSES: userAddresses,
    BLACKLIST_ADDRESSES: blacklist,
    ...
    MAX_TRADE_AGE_SECONDS: parseInt(process.env.MAX_TRADE_AGE_SECONDS || '60', 10),
    ...
    WS_RPC_URL: process.env.WS_RPC_URL as string | undefined,
